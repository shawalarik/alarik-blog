  ...
  secondDirOrFilenames.forEach(dirOrFilename => {
    ...
    if (!onlyScannerRootMd && statSync(filePath).isDirectory()) {
      ...
      if (permalink) {
        ...
        // index.md 文件在 VitePress 中以目录路径访问（如 /dir/ 而非 /dir/index），
        // 因此 key 不包含 /index 后缀，确保与 usePermalink 中的 URL 匹配逻辑一致
        // 同时注意 key 不能以 / 结尾，因为 usePermalink 第 29 行会 strip 尾部 /
        const isIndexFile = filename === "index";
        const keySuffix = isIndexFile ? "" : filename;

        const finalPermalink = standardLink(permalink);
        // 构造完整 key，当 keySuffix 为空（index.md）时避免产生尾部 "/"
        const rawKey = prefix ? `${prefix}/${keySuffix}` : keySuffix;
        permalinks[rawKey.replace(/\/$/, "")] = cleanUrls ? finalPermalink : `${finalPermalink}.html`;
      }
    }
  });
};
