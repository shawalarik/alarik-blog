...
export function VitePluginVitePressAutoPermalink(option: PermalinkOption = {}): Plugin & { name: string } {
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      ...
      // 将 permalink 重写实际文件路径（支持 .html 和无扩展名两种访问方式）
      server.middlewares.use((req, _res, next) => {
        ...
        const isHtmlRequest = reqUrl.endsWith(".html");
        const isMdRequest = reqUrl.endsWith(".md");

        // 只处理 .html 和 .md 请求（跳过静态资源等）
        if (!isHtmlRequest && !isMdRequest) return next();

        if (isHtmlRequest) reqUrl = reqUrl.replace(/\.html$/, "");

        // 确保 path 以 / 开头
        const path = "/" + reqUrl.replace(/^\//, "");
        // 如果访问链接为 permalink，则找到对应的文档路由
        const filePath = permalinks.inv[path] || permalinks.inv[`${path}.html`] || permalinks.inv[`${path}/`];
        ...
        if (realFilePath) {
          ...
          req.url = base + realFilePath + ".html" + searchPart + hashPart;
        }
